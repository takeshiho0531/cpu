...
val pc_next = MuxCase(pc_plus4, Seq(
    br_flg -> br_target,
    jmp_flg -> alu_out,
    // 0x305:mtvecにはtrap_vectorアドレスが格納
    (inst === ECALL) -> csr_regfile(0x305) // 追加
))
...
val csignals = ListLookup(inst,
    ...
    Array(
        ...
        CSSRC -> List(ALU_COPY1, OP1_RS1, OP2_X, MEN_X, REN_S, WB_CSR, CSR_C),
        CSSRCI -> List(ALU_COPY1, OP1_IMZ, OP2_X, MEN_X, REN_S, WB_CSR, CSR_C),
        ECALL -> List(ALU_X, OP1_X, OP2_X, MEN_X, REN_X, WB_X, CSR_E)  // 追加
    )
)
...
// CSRのアドレス0x342はmcauseレジスタ
val csr_addr = Mux(csr_cmd === CSR_E, 0x342.U(CSR_ADD_LEN.W), inst(31, 20)) // 追加
...
val csr_wdata = MuxCase(0.U(WORD_LEN.W), Seq(
    ...
    (csr_cmd === CSR_E) -> 11.U(WORD_LEN.W) // マシンモードからのECALL // 追加
))

when (csr_cmd > 0.U) {  // CSR命令の時
