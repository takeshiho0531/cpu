...
// IF/IDレジスタへの書き込み
id_reg_pc   := if_reg_pc
id_reg_inst := Mux((exe_br_flg || exe_jmp_flg), BUBBLE, if_inst)  // 変更

// **********
// ID Stage
// 分岐成立またはジャンプ命令時にBUBBLE化させるマルチプレクサを追加し、id_instに出力
val id_inst = Mux((exe_br_flg || exe_jmp_flg), BUBBLE, id_reg_inst)  // 追加
