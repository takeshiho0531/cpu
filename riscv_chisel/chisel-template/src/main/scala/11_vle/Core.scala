...
class Core extends Module {
    val io = IO(
        new Bundle{
            ...
            val omem = Flipped(new DmemPortIo())
    ...
    val vr_data = Mem(32, UInt(VLEN.W)) // 追加

    // ***********************
    // Instructions Fetch (IF) Stage
    ...
    val csignals = ListLookup(inst,
        ...
        Array(
            ...
            BEQ     -> List(BR_BEQ,    OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            BNE     -> List(BR_BNE,    OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            BGE     -> List(BR_BGE,    OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            BGEU    -> List(BR_BGEU,   OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            BLT     -> List(BR_BLT,    OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            BLTU    -> List(BR_BLTU,   OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_X,     CSR_X),
            ...
            CSRRS   -> List(ALU_COPY1, OP1_RSI, OP2_X,   MEN_X, REN_S, WB_CSR,   CSR_S),
            ...
            ECALL   -> List(ALU_X,     OP1_X,   OP2_X,   MEN_X, REN_S, WB_X,     CSR_E),
            VSETVLI -> List(ALU_X,     OP1_X,   OP2_X,   MEN_X, REN_S, WB_VL,    CSR_V),
            VLE     -> List(ALU_COPY1, OP1_RS1, OP2_X,   MEN_X, REN_V, WB_MWM_V, CSR_X)  // 追加
        ))
    ...
    alu_out := MuxCase(0.U(WORD_LEN.W), Seq(
        ...
        (exe_fun === ALU_SLT)   -> (op1_data.asSInt() < op1_data.asSInt()).asUInt(),
    ...
    val vsew  = vtype(4, 2)
    val vlmul = vtype(1, 0)
    val vlmax = ((VLEN.U << vlmul)) >> (vsew + 3.U(3.W)).asSInt()
    ...
    // VLE
    val v_wb_data = io.dmem.vr_data

    when (rf_wen === REN_S) {
        regfile(wb_addr) := wb_data
    }.elsewhen (rf_wen === REN_V) {
        /* VLE命令にエンコードされたEEWではなく、CSRのSEWを利用
        (本書の範囲内では常にSEW=EEWなので、実装簡略化)
        */
        ...
        /* 利用するベクトルレジスタに0-7までのid付与
        e.g.) ライトバックデータ幅が288bitなら、last_reg_id = 2.U */
        val last_reg_id = data_len / VLEN.u

        for (reg_id <- 0 to 7) { // reg_id =0~7まで8回ループ
            when (reg_id.U < last_reg_id) {
                vec_regfile(wb_addr + reg_id.U) := v_wb_data(VLEN*(reg_id+1)-1, VLEN*reg_id)
            }.elsewhen (reg_id === last_reg_id) {
                // e.g) ライトバックデータ幅が288bitなら、remainder=32bit
                val remainder    = data_len % VLEN.U
                val tail_width   = VLEN.U - remainder
                val org_reg_data = vec_regfile(wb_addr+reg_id.U)

                /* remainderの部分を0に
                [tail][xx...x] → 論理右シフト → [00...0][tail] → 左シフト → [00...0][remainder] */
                val tail_reg_data = ((org_reg_data >> remainder) << remainder)(VLEN-1, 0)

                /* tailの部分を0に
                [xx...x][remainder] → 左シフト → [remainder][00...0] → 論理右シフト → [00...0][remainder] 
                ">>"はBits型を返すため、asUInt()でUInt型へ変換 */
                ...
                val regfile(wb_addr+reg_id.U) := undisturbed_v_wb_data
            }
        }
    }
    ...
    printf(p"rs1_addr : 0x${Hexadecimal(rs1_data)}=${rs1_data}\n")
    printf(p"rs2_addr : 0x${Hexadecimal(rs2_data)}=${rs2_data}\n")
    printf(p"rs1_addr : 0x${Hexadecimal(wb_data)}\n")
